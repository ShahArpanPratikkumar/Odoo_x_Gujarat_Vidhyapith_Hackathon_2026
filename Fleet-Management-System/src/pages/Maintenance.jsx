...
const MBADGE = {
    ...
    "Overdue": { bg: "var(--danger-bg)", bd: "var(--danger)40", c: "var(--danger)" },
};

function Pill({ s }) {
    const t = MBADGE[s] || MBADGE["Scheduled"];
...
export default function Maintenance() {
    const { maintenance, searchQuery, setSearchQuery } = useFleet();
    const [filter, setFilter] = useState("All");

    const filtered = maintenance.filter(m => {
        const query = searchQuery.toLowerCase().trim();
        const matchesSearch = m.vehicle.toLowerCase().includes(query) ||
    ...
    return (
        <Layout>
            <div className="page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                <div>
                    <h1 className="page-title" style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: "1.8rem", fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>Maintenance</h1>
                    <p style={{ color: "var(--text-secondary)", fontSize: "0.85rem", marginTop: 4 }}>Track service history and schedules</p>
                </div>
            ...
            <div className="toolbar" style={{ display: "flex", gap: 12, marginBottom: 20, alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", flex: 1, maxWidth: 400 }} className="search-bar">
                    <input
                        placeholder="Search vehicle, type, technician..."
                ...
                <table className="fleet-table" style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                        <tr style={{ background: "var(--bg-secondary)" }}>
                            {["Vehicle", "Service Type", "Date", "Technician", "Cost", "Status"].map(h => (
                    ...
                    <tbody>
                        {filtered.map(m => (
                            <tr key={m.id} style={{ borderTop: "1px solid var(--border)", transition: "background .12s" }} onMouseEnter={e => e.currentTarget.style.background = "var(--bg-card-hover)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                                <td style={{ padding: "14px 18px" }}>
                                    <span style={{ fontFamily: "monospace", background: `${C}12`, color: C, padding: "2px 7px", borderRadius: 5, fontSize: "0.78rem", border: `1px solid ${C}25` }}>{m.vehicle}</span>
                                </td>
                                <td style={{ padding: "14px 18px" }}>
                                    <div style={{ fontWeight: 600, color: "var(--text-primary)", fontSize: "0.85rem" }}>{m.type}</div>
                                    <div style={{ fontSize: "0.72rem", color: "var(--text-muted)", marginTop: 2 }}>{m.priority} Priority</div>
                                </td>
                                <td style={{ padding: "14px 18px", color: "var(--text-secondary)", fontSize: "0.82rem" }}>{m.date}</td>
                                <td style={{ padding: "14px 18px", color: "var(--text-secondary)", fontSize: "0.82rem" }}>{m.technician}</td>
                                <td style={{ padding: "14px 18px", color: "var(--text-primary)", fontSize: "0.85rem", fontWeight: 600 }}>₹{m.cost.toLocaleString()}</td>
                                <td style={{ padding: "14px 18px" }}><Pill s={m.status} /></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
